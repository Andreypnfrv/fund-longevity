import type { Translation } from './types';
import { Locale } from './types';

export const globalTranslations = {
  nav: {
    home: {
      [Locale.EN]: 'Home',
      [Locale.SV]: 'Hem',
      [Locale.DE]: 'Start',
      [Locale.FR]: 'Accueil',
      [Locale.ES]: 'Inicio',
      [Locale.IT]: 'Home',
      [Locale.NL]: 'Home',
      [Locale.CS]: 'Domů',
    },
    why: {
      [Locale.EN]: 'Why',
      [Locale.SV]: 'Varför',
      [Locale.DE]: 'Warum',
      [Locale.FR]: 'Pourquoi',
      [Locale.ES]: 'Por qué',
      [Locale.IT]: 'Perché',
      [Locale.NL]: 'Waarom',
      [Locale.CS]: 'Proč',
    },
    asks: {
      [Locale.EN]: 'Asks',
      [Locale.SV]: 'Krav',
      [Locale.DE]: 'Forderungen',
      [Locale.FR]: 'Revendications',
      [Locale.ES]: 'Demandas',
      [Locale.IT]: 'Richieste',
      [Locale.NL]: 'Eisen',
      [Locale.CS]: 'Požadavky',
    },
    messages: {
      [Locale.EN]: 'Signs',
      [Locale.SV]: 'Skyltar',
      [Locale.DE]: 'Schilder',
      [Locale.FR]: 'Pancartes',
      [Locale.ES]: 'Pancartas',
      [Locale.IT]: 'Cartelli',
      [Locale.NL]: 'Borden',
      [Locale.CS]: 'Cedule',
    },
    about: {
      [Locale.EN]: 'About',
      [Locale.SV]: 'Om oss',
      [Locale.DE]: 'Über uns',
      [Locale.FR]: 'À propos',
      [Locale.ES]: 'Sobre nosotros',
      [Locale.IT]: 'Chi siamo',
      [Locale.NL]: 'Over ons',
      [Locale.CS]: 'O nás',
    },
    donate: {
      [Locale.EN]: 'Donate',
      [Locale.SV]: 'Donera',
      [Locale.DE]: 'Spenden',
      [Locale.FR]: 'Faire un don',
      [Locale.ES]: 'Donar',
      [Locale.IT]: 'Dona',
      [Locale.NL]: 'Doneren',
      [Locale.CS]: 'Darovat',
    },
  },
  footer: {
    summary: {
      [Locale.EN]: 'Fund Longevity is a global rally for life extension.',
      [Locale.SV]: 'Fund Longevity är ett multinationellt rally för livsförlängning.',
      [Locale.DE]: 'Fund Longevity ist eine globale Kundgebung für Lebensverlängerung.',
      [Locale.FR]: 'Fund Longevity est un rassemblement global pour l\'extension de la vie.',
      [Locale.ES]: 'Fund Longevity es una manifestación multinacional por la extensión de la vida.',
      [Locale.IT]: 'Fund Longevity è un raduno multinazionale per l\'estensione della vita.',
      [Locale.NL]: 'Fund Longevity is een globale bijeenkomst voor levensverlenging.',
      [Locale.CS]: 'Fund Longevity je globální rally za prodlužování života.',
    },
    discord: {
      [Locale.EN]: 'Join Discord',
      [Locale.SV]: 'Gå med i Discord',
      [Locale.DE]: 'Discord beitreten',
      [Locale.FR]: 'Rejoindre Discord',
      [Locale.ES]: 'Unirse a Discord',
      [Locale.IT]: 'Unisciti a Discord',
      [Locale.NL]: 'Word lid van Discord',
      [Locale.CS]: 'Připojit se na Discord',
    },
  },
  common: {
    join: {
      [Locale.EN]: 'Join',
      [Locale.SV]: 'Gå med',
      [Locale.DE]: 'Mitmachen',
      [Locale.FR]: 'Rejoindre',
      [Locale.ES]: 'Unirse',
      [Locale.IT]: 'Partecipa',
      [Locale.NL]: 'Doe mee',
      [Locale.CS]: 'Připojit se',
    },
    learnMore: {
      [Locale.EN]: 'Learn more',
      [Locale.SV]: 'Läs mer',
      [Locale.DE]: 'Mehr erfahren',
      [Locale.FR]: 'En savoir plus',
      [Locale.ES]: 'Saber más',
      [Locale.IT]: 'Scopri di più',
      [Locale.NL]: 'Meer informatie',
      [Locale.CS]: 'Zjistit více',
    },
    close: {
      [Locale.EN]: 'Close',
      [Locale.SV]: 'Stäng',
      [Locale.DE]: 'Schließen',
      [Locale.FR]: 'Fermer',
      [Locale.ES]: 'Cerrar',
      [Locale.IT]: 'Chiudi',
      [Locale.NL]: 'Sluiten',
      [Locale.CS]: 'Zavřít',
    },
    closeOverlay: {
      [Locale.EN]: 'Close overlay',
      [Locale.SV]: 'Stäng övertäckning',
      [Locale.DE]: 'Overlay schließen',
      [Locale.FR]: 'Fermer la superposition',
      [Locale.ES]: 'Cerrar superposición',
      [Locale.IT]: 'Chiudi sovrapposizione',
      [Locale.NL]: 'Overlay sluiten',
      [Locale.CS]: 'Zavřít překrytí',
    },
    closeMenu: {
      [Locale.EN]: 'Close menu',
      [Locale.SV]: 'Stäng meny',
      [Locale.DE]: 'Menü schließen',
      [Locale.FR]: 'Fermer le menu',
      [Locale.ES]: 'Cerrar menú',
      [Locale.IT]: 'Chiudi menu',
      [Locale.NL]: 'Menu sluiten',
      [Locale.CS]: 'Zavřít menu',
    },
    changeLanguage: {
      [Locale.EN]: 'Change language',
      [Locale.SV]: 'Byt språk',
      [Locale.DE]: 'Sprache ändern',
      [Locale.FR]: 'Changer de langue',
      [Locale.ES]: 'Cambiar idioma',
      [Locale.IT]: 'Cambia lingua',
      [Locale.NL]: 'Taal wijzigen',
      [Locale.CS]: 'Změnit jazyk',
    },
    copyPageLink: {
      [Locale.EN]: 'Copy link',
      [Locale.SV]: 'Kopiera länk',
      [Locale.DE]: 'Link kopieren',
      [Locale.FR]: 'Copier le lien',
      [Locale.ES]: 'Copiar enlace',
      [Locale.IT]: 'Copia link',
      [Locale.NL]: 'Link kopiëren',
      [Locale.CS]: 'Kopírovat odkaz',
    },
    pageLinkCopied: {
      [Locale.EN]: 'Copied!',
      [Locale.SV]: 'Kopierat!',
      [Locale.DE]: 'Kopiert!',
      [Locale.FR]: 'Copié !',
      [Locale.ES]: '¡Copiado!',
      [Locale.IT]: 'Copiato!',
      [Locale.NL]: 'Gekopieerd!',
      [Locale.CS]: 'Zkopírováno!',
    },
  },
  forms: {
    firstName: {
      [Locale.EN]: 'First name',
      [Locale.SV]: 'Förnamn',
      [Locale.DE]: 'Vorname',
      [Locale.FR]: 'Prénom',
      [Locale.ES]: 'Nombre',
      [Locale.IT]: 'Nome',
      [Locale.NL]: 'Voornaam',
      [Locale.CS]: 'Jméno',
    },
    city: {
      [Locale.EN]: 'City',
      [Locale.SV]: 'Stad',
      [Locale.DE]: 'Stadt',
      [Locale.FR]: 'Ville',
      [Locale.ES]: 'Ciudad',
      [Locale.IT]: 'Città',
      [Locale.NL]: 'Stad',
      [Locale.CS]: 'Město',
    },
    email: {
      [Locale.EN]: 'Email',
      [Locale.SV]: 'E-post',
      [Locale.DE]: 'E-Mail',
      [Locale.FR]: 'E-mail',
      [Locale.ES]: 'Correo',
      [Locale.IT]: 'Email',
      [Locale.NL]: 'E-mail',
      [Locale.CS]: 'E-mail',
    },
    emailPlaceholder: {
      [Locale.EN]: 'you@example.com',
      [Locale.SV]: 'du@exempel.se',
      [Locale.DE]: 'name@beispiel.de',
      [Locale.FR]: 'vous@exemple.fr',
      [Locale.ES]: 'tu@ejemplo.com',
      [Locale.IT]: 'tu@esempio.it',
      [Locale.NL]: 'jij@voorbeeld.nl',
      [Locale.CS]: 'vy@priklad.cz',
    },
    phone: {
      [Locale.EN]: 'Phone',
      [Locale.SV]: 'Telefon',
      [Locale.DE]: 'Telefon',
      [Locale.FR]: 'Téléphone',
      [Locale.ES]: 'Teléfono',
      [Locale.IT]: 'Telefono',
      [Locale.NL]: 'Telefoon',
      [Locale.CS]: 'Telefon',
    },
    company: {
      [Locale.EN]: 'Company',
      [Locale.SV]: 'Företag',
      [Locale.DE]: 'Unternehmen',
      [Locale.FR]: 'Entreprise',
      [Locale.ES]: 'Empresa',
      [Locale.IT]: 'Azienda',
      [Locale.NL]: 'Bedrijf',
      [Locale.CS]: 'Společnost',
    },
    submit: {
      [Locale.EN]: 'Send',
      [Locale.SV]: 'Skicka',
      [Locale.DE]: 'Senden',
      [Locale.FR]: 'Envoyer',
      [Locale.ES]: 'Enviar',
      [Locale.IT]: 'Invia',
      [Locale.NL]: 'Versturen',
      [Locale.CS]: 'Odeslat',
    },
    subscribe: {
      [Locale.EN]: 'Subscribe',
      [Locale.SV]: 'Prenumerera',
      [Locale.DE]: 'Abonnieren',
      [Locale.FR]: "S'inscrire",
      [Locale.ES]: 'Suscribirse',
      [Locale.IT]: 'Iscriviti',
      [Locale.NL]: 'Abonneren',
      [Locale.CS]: 'Odebírat',
    },
    subscribeToEmail: {
      [Locale.EN]: 'Subscribe to email',
      [Locale.SV]: 'Prenumerera på e-post',
      [Locale.DE]: 'Per E-Mail abonnieren',
      [Locale.FR]: 'S’inscrire par e-mail',
      [Locale.ES]: 'Suscribirse por correo',
      [Locale.IT]: 'Iscriviti via email',
      [Locale.NL]: 'Abonneren via e-mail',
      [Locale.CS]: 'Odebírat e-mailem',
    },
    joinOffline: {
      [Locale.EN]: "I'll join offline",
      [Locale.SV]: 'Jag kommer delta offline',
      [Locale.DE]: 'Ich nehme offline teil',
      [Locale.FR]: 'Je participerai en présentiel',
      [Locale.ES]: 'Participaré en persona',
      [Locale.IT]: 'Parteciperò di persona',
      [Locale.NL]: 'Ik doe offline mee',
      [Locale.CS]: 'Přijdu osobně',
    },
    canTakePart: {
      [Locale.EN]: 'I can take part',
      [Locale.SV]: 'Jag kan delta',
      [Locale.DE]: 'Ich kann teilnehmen',
      [Locale.FR]: 'Je peux participer',
      [Locale.ES]: 'Puedo participar',
      [Locale.IT]: 'Posso partecipare',
      [Locale.NL]: 'Ik kan deelnemen',
      [Locale.CS]: 'Mohu se zúčastnit',
    },
    intent: {
      [Locale.EN]: 'Intent',
      [Locale.SV]: 'Avsikt',
      [Locale.DE]: 'Absicht',
      [Locale.FR]: 'Intention',
      [Locale.ES]: 'Intención',
      [Locale.IT]: 'Intenzione',
      [Locale.NL]: 'Intentie',
      [Locale.CS]: 'Záměr',
    },
    joinInPerson: {
      [Locale.EN]: "I want to join in-person",
      [Locale.SV]: 'Jag vill delta personligen',
      [Locale.DE]: 'Ich möchte persönlich teilnehmen',
      [Locale.FR]: 'Je veux participer en présentiel',
      [Locale.ES]: 'Quiero participar en persona',
      [Locale.IT]: 'Voglio partecipare di persona',
      [Locale.NL]: 'Ik wil persoonlijk deelnemen',
      [Locale.CS]: 'Chci se zúčastnit osobně',
    },
    joinOnline: {
      [Locale.EN]: "I want to join online",
      [Locale.SV]: 'Jag vill delta online',
      [Locale.DE]: 'Ich möchte online teilnehmen',
      [Locale.FR]: 'Je veux participer en ligne',
      [Locale.ES]: 'Quiero participar en línea',
      [Locale.IT]: 'Voglio partecipare online',
      [Locale.NL]: 'Ik wil online deelnemen',
      [Locale.CS]: 'Chci se zúčastnit online',
    },
    canTakePartInOrganisation: {
      [Locale.EN]: 'I can take part in organisation',
      [Locale.SV]: 'Jag kan delta i organisationen',
      [Locale.DE]: 'Ich kann bei der Organisation mitwirken',
      [Locale.FR]: 'Je peux participer à l\'organisation',
      [Locale.ES]: 'Puedo participar en la organización',
      [Locale.IT]: 'Posso partecipare all\'organizzazione',
      [Locale.NL]: 'Ik kan meedoen aan de organisatie',
      [Locale.CS]: 'Mohu se podílet na organizaci',
    },
    media: {
      [Locale.EN]: 'Media',
      [Locale.SV]: 'Media',
      [Locale.DE]: 'Medien',
      [Locale.FR]: 'Médias',
      [Locale.ES]: 'Medios',
      [Locale.IT]: 'Media',
      [Locale.NL]: 'Media',
      [Locale.CS]: 'Média',
    },
    ideallyUrl: {
      [Locale.EN]: 'Ideally url',
      [Locale.SV]: 'Helst url',
      [Locale.DE]: 'Idealerweise URL',
      [Locale.FR]: 'Idéalement une URL',
      [Locale.ES]: 'Idealmente URL',
      [Locale.IT]: 'Ideally url',
      [Locale.NL]: 'Bij voorkeur url',
      [Locale.CS]: 'Ideálně URL',
    },
    sending: {
      [Locale.EN]: 'Sending...',
      [Locale.SV]: 'Skickar...',
      [Locale.DE]: 'Wird gesendet...',
      [Locale.FR]: 'Envoi en cours...',
      [Locale.ES]: 'Enviando...',
      [Locale.IT]: 'Invio in corso...',
      [Locale.NL]: 'Verzenden...',
      [Locale.CS]: 'Odesílám...',
    },
    subscribing: {
      [Locale.EN]: 'Subscribing...',
      [Locale.SV]: 'Prenumererar...',
      [Locale.DE]: 'Wird abonniert...',
      [Locale.FR]: 'Inscription...',
      [Locale.ES]: 'Suscribiendo...',
      [Locale.IT]: 'Iscrizione...',
      [Locale.NL]: 'Bezig met abonneren...',
      [Locale.CS]: 'Odebírání...',
    },
    emailInvalid: {
      [Locale.EN]: 'Please enter a valid email address',
      [Locale.SV]: 'Vänligen ange en giltig e-postadress',
      [Locale.DE]: 'Bitte geben Sie eine gültige E-Mail-Adresse ein',
      [Locale.FR]: 'Veuillez entrer une adresse e-mail valide',
      [Locale.ES]: 'Por favor, introduzca un correo válido',
      [Locale.IT]: 'Inserisci un indirizzo email valido',
      [Locale.NL]: 'Voer een geldig e-mailadres in',
      [Locale.CS]: 'Zadejte prosím platnou e-mailovou adresu',
    },
    successMessage: {
      [Locale.EN]: 'Thank you! Your submission was successful.',
      [Locale.SV]: 'Tack! Din inlämning lyckades.',
      [Locale.DE]: 'Danke! Ihre Übermittlung war erfolgreich.',
      [Locale.FR]: 'Merci ! Votre envoi a réussi.',
      [Locale.ES]: '¡Gracias! Su envío fue exitoso.',
      [Locale.IT]: 'Grazie! L\'invio è andato a buon fine.',
      [Locale.NL]: 'Bedankt! Uw inzending is gelukt.',
      [Locale.CS]: 'Děkujeme! Vaše přihlášení bylo úspěšné.',
    },
    errorMessage: {
      [Locale.EN]: 'Something went wrong. Please try again.',
      [Locale.SV]: 'Något gick fel. Vänligen försök igen.',
      [Locale.DE]: 'Etwas ist schiefgelaufen. Bitte versuchen Sie es erneut.',
      [Locale.FR]: 'Une erreur s\'est produite. Veuillez réessayer.',
      [Locale.ES]: 'Algo salió mal. Por favor, inténtelo de nuevo.',
      [Locale.IT]: 'Qualcosa è andato storto. Riprova.',
      [Locale.NL]: 'Er ging iets mis. Probeer het opnieuw.',
      [Locale.CS]: 'Něco se pokazilo. Zkuste to prosím znovu.',
    },
    fieldRequired: {
      [Locale.EN]: 'This field is required',
      [Locale.SV]: 'Detta fält är obligatoriskt',
      [Locale.DE]: 'Dieses Feld ist erforderlich',
      [Locale.FR]: 'Ce champ est obligatoire',
      [Locale.ES]: 'Este campo es obligatorio',
      [Locale.IT]: 'Questo campo è obbligatorio',
      [Locale.NL]: 'Dit veld is verplicht',
      [Locale.CS]: 'Toto pole je povinné',
    },
  },
} as const satisfies Record<string, Record<string, Translation>>;

export type GlobalTranslationKeys = typeof globalTranslations;

